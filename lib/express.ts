const expressDependencies = {
	...
	saveDev: ['@types/express', '@types/body-parser', '@types/cors', '@types/helmet','ts-node',"@types/node"],
	dirs: ['src','views','public','routes','controllers','middlewares','models','services','config','utils']
	
}

export function getDependencies(appType: string, dbOptions: { client: string; db: string }) {
...
export function getDatabaseOptions(appType: string): Promise<{ client: string; db: string }> {
	return new Promise((resolve) => {
		...
		if (appType === 'express-db' || appType === 'express-db-testing') {
			...
			rl.question('Enter your choice (1-3): ', (choice: string) => {
				...
				rl.question('Select the database type (mongodb, postgres, mysql): ', (db: string) => {
					resolve({ client, db });
					rl.close();
				});
			});
