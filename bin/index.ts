...
function selectAppType(): Promise<string> {
    return new Promise((resolve) => {
        ...
        console.log("3. Express Application with Database and Testing (Jest, Playwright)");
...
async function installDependencies(projectName: string, appType: string, dbOptions: { client: string; db: string }) {
    return new Promise((resolve, reject) => {
...
function createFiles(projectName: string, appType: string, dbOptions: { client: string; db: string }) {
    const projectPath = path.join(process.cwd(), projectName);
    ...
    if (appType === 'express-db') {
        ...
        fs.writeFileSync(path.join(projectPath, 'src', 'db.ts'), `
...
export default connectDB;
        ...
        fs.writeFileSync(path.join(projectPath, 'jest.config.js'), `
module.exports = {
    preset: 'ts-jest',
    testEnvironment: 'node',
};
        `);
        fs.writeFileSync(path.join(projectPath, 'playwright.config.ts'), `
